...
use sessions::{AuthorizatinArea0, Session, AA};
...
where
    ...
    T: Tpm,
{
    Ok(run_command_with_handles(cmd, CmdT::Handles::default(), AuthorizatinArea0, tpm)?.0)
}

fn marshal_auth_size(auth_offset: usize, buffer: &mut [u8]) -> TpmResult<usize> {
...
pub fn write_command_sessions<
    ...
    Y: Session,
    Z: Session,
    AuthorizationArea: AA<X, Y, Z>,
>(
    sessions: &AuthorizationArea,
    buffer: &mut [u8],
) -> TpmResult<usize> {
...
>(
    ...
    cmd_sessions: AuthorizationArea,
    tpm: &mut T,
